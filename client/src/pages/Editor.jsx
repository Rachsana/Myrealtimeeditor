...
export default function Editor() {
  ...
  useEffect(() => {
    ...
    const socket = io(import.meta.env.VITE_SERVER_URL || "http://localhost:5000", {
      ...
      auth: { token }, // verified server-side before any doc access is granted
    });
  ...
  // Broadcast local cursor position to others
  useEffect(() => {
  ...
  const openVersions = async () => {
    ...
    setAiSummary("");
    try {
      const { data } = await api.get(`/docs/${id}/versions`);
      setVersions(data);
    } catch (err) {
  ...
  const handleSummarize = async () => {
    ...
    setAiSummary("");
    try {
      const { data } = await api.get(`/docs/${id}/summarize`);
      setAiSummary(data.summary);
    } catch (err) {
      console.error("Failed to summarize changes:", err);
      setAiSummary("Couldn't generate a summary right now — try again in a moment.");
    } finally {
  ...
  const handleRestore = async (versionId) => {
    if (!window.confirm("Restore this version? Your current content will be saved as a version too, so you can undo this.")) return;
    try {
      const { data } = await api.post(`/docs/${id}/restore/${versionId}`);
      setContent(data.content);
      socketRef.current?.emit("send-changes", { docId: id, delta: data.content });
      socketRef.current?.emit("save-doc", { docId: id, content: data.content });
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.topbar}>
        ...
        <button style={styles.shareBtn} onClick={openVersions}>History</button>
...
>
    {Object.values(remoteCursors).map((cursor) => (
      ...
      {showVersions && (
        <div style={styles.modalBg} onClick={() => setShowVersions(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <h2 style={styles.modalTitle}>Version history</h2>

            <div style={{ marginBottom: 14, padding: 12, background: "#f7f7fb", borderRadius: 8 }}>
              <button
                style={{ ...styles.copyBtn, marginBottom: aiSummary || aiSummaryLoading ? 8 : 0 }}
                onClick={handleSummarize}
                disabled={aiSummaryLoading}
              >
                {aiSummaryLoading ? "Summarizing…" : "✨ Summarize latest changes (AI)"}
              </button>
              {aiSummary && (
                <p style={{ fontSize: 13, color: "#333", margin: 0, lineHeight: 1.5 }}>{aiSummary}</p>
              )}
            </div>

            {versionsLoading && <p style={{ fontSize: 13, color: "#9b9b9b" }}>Loading…</p>}
            {!versionsLoading && versions.length === 0 && (
              <p style={{ fontSize: 13, color: "#9b9b9b" }}>
                No saved versions yet — versions are snapshotted automatically as you edit.
              </p>
            )}
            <div style={{ ...styles.collabList, maxHeight: 320, overflowY: "auto" }}>
              {versions.map((v) => (
                <div key={v._id} style={styles.versionRow}>
                  <div style={{ flex: 1 }}>
                    <div style={styles.collabName}>
                      {new Date(v.savedAt).toLocaleString()}
                    </div>
                    <div style={styles.collabEmail}>
                      {v.savedBy?.name ? `by ${v.savedBy.name} — ` : ""}
                      {v.preview || "(empty)"}
                      {v.preview?.length === 120 ? "…" : ""}
                    </div>
                  </div>
                  <button style={styles.copyBtn} onClick={() => handleRestore(v._id)}>Restore</button>
                </div>
              ))}
            </div>
            <div style={{ textAlign: "right", marginTop: 16 }}>
              <button style={styles.doneBtn} onClick={() => setShowVersions(false)}>Done</button>
            </div>
          </div>
        </div>
      )}
...
const styles = {
  ...
  versionRow: { display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid #f0f0f0" },
  collabInfo: { flex: 1 },
